...
fn main() {
    ...
    let taken_disk_space = file_system.get("/").unwrap();
    let remaining_disk_space = total_disk_space - taken_disk_space;
    let amount_needed_to_delete = needed_disk_space - remaining_disk_space;

    let mut res = file_system
        .iter()
        .filter_map(|(_, value)| {
            if value >= &amount_needed_to_delete {
                Some(value)
            } else {
                None
            }
