...
// note: don't discard the pool of previously visited tile indices, they are used to check which tiles have already been explored
fn explore(
    height_map: Vec<i32>,
    ...
    let mut visited_tile_indices: Vec<usize> = starting_positions.clone();
    let mut new_tiles_previous_turn: Vec<usize> = starting_positions;
    let mut i: i32 = 0;
        ...
        for visited_tile_index in &new_tiles_previous_turn {
            ...
            for direction_offset in tile_direction_offsets {
                ...
                if visited_tile_indices.contains(&(offset_tile_index as usize))
                    ...
                    || new_tiles_previous_turn.contains(&(offset_tile_index as usize))
                {
                    continue;
                }
...
fn main() {
    ...
    let starting_positions = grid_tiles
        .iter()
        .enumerate()
        .filter_map(|(i, height)| {
    ...
    let path_length = explore(grid_tiles.clone(), map_width, starting_positions, end_index);
