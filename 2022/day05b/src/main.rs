...
fn execute_steps(steps: &str, c: Vec<Vec<char>>) -> Vec<Vec<char>> {
    ...
    steps.lines().for_each(|line| {
        ...
        for i in 0..amount {
            let grab_idx = amount - i - 1;

            let grabbed_item = new_columns[from_index][grab_idx];
            new_columns[to_index].insert(0, grabbed_item);
        }

        new_columns[from_index] = new_columns[from_index][amount..].to_vec();
    });
